...
export default {
  ...
  setup() {
    ...
    const taskListOverview = reactive([
      { name: 'Semua', length: computed(() => taskLists.all.length) },
      { name: 'Saat Ini', length: computed(() => taskLists.current.length) },
      { name: 'Selesai', length: computed(() => taskLists.completed.length) }
    ])

    const tasksInView = computed(() => {
      if (state.currentView === 'Saat Ini') {
        return state.taskList.filter(item => item.complete === false)
      } else if (state.currentView === 'Selesai') {
